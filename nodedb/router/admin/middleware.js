(function(){
    ...
    function middleware (app, mongoDB, config){
        ...
        app.use(async (ctx, next) => {
            ...
            ctx.utils.log(ctx.tnxId, JSON.stringify({ dated: start, method: ctx.method, url: ctx.url, time: ms, status: ctx.status, request: JSON.stringify(ctx.request.body), response: JSON.stringify(ctx.body)}), "Middleware,"+ statusTag, ctx.config);
        });

        //Block Direct Access
        app.use(getMethod(methodName.serverBlocker));

        //app.use(swagger.koa({route: config.moduleName + "/definition",swagger: "./swagger.json"}));
